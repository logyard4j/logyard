...
/** Shared event traversal and profile policy for text and UTF-8 output buffers. */
final class JsonEventWriter {
    ...
    private final JsonExceptionWriter exceptions;

    JsonEventWriter(ResourceAttributes resource, JsonProfile profile, JsonTokenBuffer buffer) {
        json = new JsonWriter(buffer);
        exceptions = new JsonExceptionWriter(json);
