...
final class VerificationFixtures {
    ...
    static LogyardConfig extensionConfig(String filterConfig, String outputConfig, int capacity) {
        return parse(extensionText(
                "test-output",
                "com.logyard4j.tests.extensions.TestOutputProvider",
                outputConfig,
                "test-filter",
                filterConfig,
                capacity));
    ...
    static String extensionText(
        ...
        return """
                ...
                implementation = "com.logyard4j.tests.extensions.TestFormatterProvider"
                [formatters.line.config]
                prefix = "fmt:"
                [encoders.record]
                type = "custom"
                provider = "test-encoder"
                implementation = "com.logyard4j.tests.extensions.TestEncoderProvider"
                [encoders.record.config]
                tag = "enc"
                [enrichers.add]
                provider = "test-enricher"
                implementation = "com.logyard4j.tests.extensions.TestEnricherProvider"
