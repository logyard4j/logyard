package com.logyard4j.examples.vertx;
...
final class VerificationPort {
    ...
    static void publish(int port) {
        ...
        if (destination == null || destination.isBlank()) {
            System.out.println("Vert.x example listening on " + port);
            return;
        }
        try {
            Files.writeString(Path.of(destination), Integer.toString(port));
        } catch (IOException failure) {
            throw new IllegalStateException("could not publish the Vert.x verification port", failure);
        }
    }
}
